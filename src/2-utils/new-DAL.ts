...
export class DAL {
    ...
    */
    async select(tableName: string): Promise<any> {
        ...
        const [rows] = await pool.query(`SELECT * FROM ${tableName}`); // Use backticks for table names
        return rows;
    }
    ...
    */
    async selectWhere(tableName: string, id: number): Promise<any> {
        const pool = await this.initializeDbPool();
     ...
     */
    async post(obj: object, tableName: string): Promise<any> {
        const pool = await this.initializeDbPool();
    ...
    async update(obj: object, tableName: string): Promise<any> {
        const pool = await this.initializeDbPool(); 
    ...
    async selectJoin(tableName: string, joins: Join[]): Promise<any> {
        const pool = await this.initializeDbPool();
